...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    for f in images:
        ...
        label = med_reshape(label, 
                            new_shape=(label.shape[0], y_shape, z_shape)).astype(int)

        # TASK: Why do we need to cast label to int?
        # ANSWER: The loss functions will expect label as int, also label values changed their datatype when reshaped is done.
